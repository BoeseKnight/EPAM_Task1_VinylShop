...
public class UserService {
    private final static DAO<User> userDAO = new UserDAO();
    ...
    public User findByEmail(String email){
        User userToFind = userDAO.getAll().stream()
    ...
    public User findByEmailAndPassword(String email, String password){
        User userToFind = userDAO.getAll().stream()
                .filter(user -> email.equals(user.getEmailAddress()) && password.equals(user.getPassword()))
                .findAny()
                .orElse(null);
        return userToFind;
    }
